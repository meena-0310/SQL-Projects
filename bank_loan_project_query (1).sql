...
begin
if new.cibil_score>900 then insert into remark_criteria (loan_id,loan_amount,cibil_score,cibil_score_sts)
values(new.loan_id,new.loan_amount,new.cibil_score,'High cibil score');
elseif new.cibil_score>750 then insert into remark_criteria (loan_id,loan_amount,cibil_score,cibil_score_sts)
values(new.loan_id,new.loan_amount,new.cibil_score,'No penalty');
elseif new.cibil_score>250 then insert into remark_criteria (loan_id,loan_amount,cibil_score,cibil_score_sts)
values(new.loan_id,new.loan_amount,new.cibil_score,'Penalty customers');
elseif new.cibil_score<=0 then insert into remark_criteria (loan_id,loan_amount,cibil_score,cibil_score_sts)
values(new.loan_id,new.loan_amount,new.cibil_score,'Reject customer (Loan not apply)');
end if;
end $$
delimiter ;

drop trigger loan_amount_check;
delimiter $$
...
select * from loan_sts;
select * from remark_criteria;

-- task 1

create table cibil_score_det select l.loan_id,l.customer_id,l.loan_amount,l.loan_amount_term,l.cibil_score,r.cibil_score_sts
from loan_sts as l inner join remark_criteria as r on l.loan_id=r.loan_id;

drop table cibil_score_det;

select * from cibil_score_det;
select * from income_sts;
...
create table bank_loan_det select c.customer_id,i.applicantincome,i.property_area,c.cibil_score,c.loan_amount,ci.gender,ci.customer_name,cs.state,l.region,l.region_id
from cibil_score_det as c inner join income_sts as i on c.loan_id=i.loan_id inner join country_state as cs on
c.customer_id=cs.customer_id inner join cus_info as ci on c.loan_id=ci.loan_id inner join loan_amount_det as l on 
c.customer_id=l.customer_id;

drop table bank_loan_det;


select* from bank_loan_det;
select* from loan_amount_det;

drop table loan_amount_det;
drop table bank_loan_det;


create table loan_amount_det 
select cs.customer_id,r.region,r.region_id,cs.state from region_info as r left join country_state as cs on r.region_id=cs.region_id; 
drop table loan_amount_det;
select*from loan_amount_det;

-- task 3
drop table loan_interest_det;
create table loan_interest_det select *,
...
case when applicantincome<5000 and property_area='Rural' then '5'
...
when applicantincome<5000 and property_area='Semiurban' then '2.5'
  ...
  describe loan_interest_det;
  select*from loan_interest_det;
 delete from loan_interest_det where loan_amount ='Loan still processing'; 
  alter table loan_interest_det modify loan_amount varchar(90);
drop table loan_interest_det;
 select* from bank_loan_det;
...
select * from loan_interest_det;
 ...
 round((loan_amount*monthly_interest/100),0)as monthly_interest_amount,
 round((loan_amount*monthly_interest/100)*12,0) as Annual_interest_amount
from loan_interest_det;
...
select * from monthly_intrest_amount;
...
begin
select c.customer_id,i.applicantincome,i.property_area,c.cibil_score,ci.gender,ci.customer_name,cs.state ,l.region,l.region_id
from cibil_score_det as c inner join income_sts as i on c.loan_id=i.loan_id inner join country_state as cs on
c.customer_id=cs.customer_id inner join cus_info as ci on c.loan_id=ci.loan_id inner join loan_amount_det as l on 
c.customer_id=l.customer_id;
...
select state,count(customer_id) as no_of_cus from bank_loan_det group by state;

select b.state,sum(c.loan_amount) as total_loan_amount from bank_loan_det as b inner join cibil_score_det as c on b.customer_id=c.customer_id
group by b.state;

select state,sum(applicantincome) as total_income from bank_loan_det group by state;

select customer_name,
right(customer_name,4) as customer_last_name from loan_interest_det where customer_name like '% ____';

end $$
delimiter ;
call  loan_details();
drop procedure loan_details;
